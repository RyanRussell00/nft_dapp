...
class App extends Component {
    ...
    async addExistingToken(tokenId) {
        ...
        if (this.state.account === owner) {
            this.setState({
                colors: [...this.state.colors, tokenId]
            })
        }
    ...
    async transfer(to, tokenId) {
        console.log("Transfer: ", this.state.account, to, tokenId);
        const owner = await this.getOwner(tokenId);
        console.log(owner);
        if (this.state.account.toString() !== owner.toString()) {
            window.alert("You don't own that tokenId NFT");
            return;
        }
        const transfer = await this.state.contract.methods.safeTransferFrom(this.state.account, to, tokenId).send(
            {from: this.state.account}
        );
