...
class App extends Component {
    async componentDidMount() {
        ...
        await this.deployContractAsUser();
    }

    // Prompts Metamask to open and ask for user to connect application with MetaMask
    async loadWeb3() {
        if (window.ethereum) {
    ...
    // Built this function following SO answer: https://ethereum.stackexchange.com/a/11097
    async deployContractAsUser() {
        const web3 = window.web3;
        // let contractFile = JSON.parse("../abis/Color.json");

        let byteCodeOfContract = Color["bytecode"];

        let SampleContract = new web3.eth.Contract(Color.abi);

        console.log("Current account: " + this.state.account);

        SampleContract.deploy({
            data: byteCodeOfContract,
        })
            .send(
                {
        ...
        return (
            <div>
                <nav className="navbar navbar-dark fixed-top bg-dark flex-md-nowrap p-0 shadow">
                    ...
                    <p className="text-white px-3 py-1">{this.state.account}</p>
                </nav>
